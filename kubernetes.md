...
we need to visit official documentation of kubernetes for installation
...
```
sudo apt-get update
sudo apt-get install -y kubeadm=1.27.0-00 kubelet=1.27.0-00 kubectl=1.27.0-00
...
**we need to run kubeadm init now**
```
...
```
***now take the given by kubeadmin tool and run it on worker nodes**
```
